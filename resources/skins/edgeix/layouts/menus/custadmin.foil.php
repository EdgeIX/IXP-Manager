<?php
/**
 * EdgIX skin: Customer admin (custadmin) top navbar.
 *
 * Changes from upstream:
 * - Customer Details / Associate Customers gated on ixp_fe.customer.details_public
 * - Entire "Customer Information" dropdown hidden when all items disabled
 */
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav mr-auto mt-2 mt-lg-0">
            ...
            <?php
                $showDetailsPublic = config( 'ixp_fe.customer.details_public' );
                $showDocstore = !config( 'ixp_fe.frontend.disabled.docstore' ) && \IXP\Models\DocstoreDirectory::getHierarchyForUserClass( \IXP\Models\User::AUTH_CUSTADMIN );
                $showMyDocs   = !config( 'ixp_fe.frontend.disabled.docstore_customer' ) && \IXP\Models\DocstoreCustomerFile::getListingForAllDirectories( Auth::getUser()->custid, \IXP\Models\User::AUTH_CUSTADMIN );
            ?>

            <?php if( $showDetailsPublic || $showDocstore || $showMyDocs ): ?>
